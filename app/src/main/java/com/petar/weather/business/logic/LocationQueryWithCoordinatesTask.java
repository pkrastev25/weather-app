...
public class LocationQueryWithCoordinatesTask
        extends AAsyncTask<String, List<NLocation>> {

    public LocationQueryWithCoordinatesTask(AAsyncTaskListener listener) {
        super(listener);
    }
    ...
    protected List<NLocation> doInBackground(String... params) {
        try {
            return ApiLogic.getInstance().getLocationQueryResultWithCoordinates(params[0]);
        } catch (Exception e) {
            mException = e;
        }
        return null;
